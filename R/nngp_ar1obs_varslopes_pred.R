...
pred_space <- function(X_pred, coords_pred, time_pred, y_obs, mu_obs, 
  ...
  # Check if coords_pred is an unobserved site
  if (length(pred_site_index) == 0) {
    ...
    if (time_pred > 1) {
      ...
      for (t in 2:max_t_krig) {
        e_t <- y_pred_tm1 - X_pred[t - 1,] %*% beta
        mu_t <- X_pred[t,] %*% beta + rho * e_t
        pars_t <- cond_mvn_par(mu_t, y_obs[t,], mu_obs[t,], sigma, l, tau, 
                               coords, cov_function)
        pred <- pars_t$mu
        y_pred_tm1 <- pred
      }
    ...
    for (t in (T + 1):time_pred) {
      ...
      w <- rnorm(n) %*% L
      e_t <- y_pred_tm1 - 
        (beta[1] * X_pred[t - 1, 1] + beta_s[pred_site_index] * X_pred[t - 1, 2])
      pred <- beta[1] * X_pred[t, 1] + beta_s[pred_site_index] * X_pred[t, 2] + 
        rho * e_t + w[pred_site_index]
      y_pred_tm1 <- pred 
    }
  }
